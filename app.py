...
# Blueprints (primero intenta en routes/, luego raíz)
try:
    from routes.documentos import documentos_bp  # si está en routes/documentos.py
except ImportError:
    from documentos import documentos_bp  # fallback si está en la raíz como documentos.py
...
def create_app() -> Flask:
    ...
    def env_info():
        data = {
            ...
            "DB_NAME": os.getenv("DB_NAME", os.getenv("MYSQLDATABASE", os.getenv("MYSQL_DATABASE", ""))),
    ...
    def test_db():
        try:
            ...
            return jsonify({"status": "ok", "total_documents": int(total)})
    ...
    def resaltar_proxy():
        ...
        if r.status_code != 200:
            try:
                ...
                err = {"error": r.text}
            return jsonify(
                {"error": err.get("error", "Error en servicio de resaltado"), "status": r.status_code}
            ), 502
